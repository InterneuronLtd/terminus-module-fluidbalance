...
export class UpsertTransactionManager {
    ...
    save(onSuccess?: (savedDetails: UpsertEntity[]) => void, onError?: (err: any) => void) {
        ...
        //this.appService.logToConsole(JSON.stringify(this.entities));

        this.apiServiceObj.postRequest(url, this.entities)
            .pipe(takeUntil(this.destroy$))
            .subscribe((newEntityDetails: UpsertEntity[]) => {

               // this.appService.logToConsole('Saved Details');
               // this.appService.logToConsole(newEntityDetails);

                this.entities = null;

                this.savedDetails = newEntityDetails;
                if (onSuccess)
                    onSuccess(newEntityDetails);
            },
                (err) => {
                   // this.appService.logToConsole('Error Details');
